...
class SimpleNet(nn.Module):
    def __init__(self, layer, embed_dimension, std1=0.015, std2=0.05, hidden = 4, dataset='cifar10', device='cpu'):
        ...
        self.discriminator = Discriminator(embed_dimension*16*16, n_layers=2, hidden=hidden)
    ...
    def forward(self, features, train=False):
        ...
        # Projection (adapter)
        true_feats = self.projector(features).reshape(len(features), -1)
        ...
        if train:
            ...
            if self.std1 != 0:
                N1 = torch.normal(0, self.std1, true_feats.shape).to(self.device)
                N1 = (N1 * noise_one_hot.unsqueeze(-1)).sum(1)
            N2 = torch.normal(0, self.std2, true_feats.shape).to(self.device)
            N2 = (N2 * noise_one_hot.unsqueeze(-1)).sum(1)
            if self.std1 != 0:
            ...
            loss = crossentropy(scores.squeeze(), Y.to(self.device))
            return loss
